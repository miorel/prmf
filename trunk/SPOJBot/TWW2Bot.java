...
class TWW2BotListener extends Thread {
	...
	private boolean processMessage(String sender, String target, String message)
	{
		...
		if(message.charAt(0) == '!')
		{
			...
			************************************************/
			...
			{
				if(param.length == 1)
					this.sendMessage(target, "My commands are: HELP, GOTOPRACTICE,SPOJ, SPOJADAY, SPOJAWEEK, SPOJAHOUR,SPOJLINK and KARMABOMB; type \"!help [cmd]\" for more help");
				else 
				{
					//SO META
			...
			{
				String name = param[1];
				this.sendMessage(target, getLastSpojSolve(name));
			}  else if(param[0].equalsIgnoreCase("lastsub"))
			{
				String name = param[1];
				this.sendMessage(target, getLastSpojSub(name));
			...
			}else if(param[0].equalsIgnoreCase("spojaweek"))
			{
				...
				this.sendMessage(target, lists[1].toString() + " are terrible human beings and only have " + getDaysTillSunday() + " and " + getTimeTillMidnight() + " left!");
			}   else if(param[0].equalsIgnoreCase("spojahour"))
			{
				...
				this.sendMessage(target, lists[1].toString() + " only have " + getTimeTillHour() + " left!");
	...
	private String[] spojaweek()
	{
		...
		for(String s : spojadayUsers)
		{
			...
			if(checkIfSameWeek(rightNow, dateOfSolve))
			{
				haveSpojadayed.add(s);
			} else {
				notSpojadayed.add(s);
	...
	private String[] spojahour()
	{
		...
		for(String s : spojadayUsers)
		{
			...
			if(checkIfSameHour(rightNow, dateOfSolve))
			{
				haveSpojadayed.add(s);
			} else {
				notSpojadayed.add(s);
	...
	private boolean checkIfSameWeek(Date d1, Date d2)
	{
		Calendar calc = new GregorianCalendar();
		calc.setTime(d1);
	...
	private boolean checkIfSameHour(Date d1, Date d2)
	{
		Calendar calc = new GregorianCalendar();
		calc.setTime(d1);
	...
	private String getDaysTillSunday()
    {
            Calendar now = new GregorianCalendar();
            now.setTime(new Date());
	...
	private String getTimeTillHour()
    {
            Calendar now = new GregorianCalendar();
            now.setTime(new Date());
            int min = 59 - now.get(Calendar.MINUTE);
	...
	public String getLastSpoj()
	{
		...
		Date bestDate = getDateOfLastSolve("naonao1234");
		for(int i = 0 ; i < spojadayUsers.size();i++)
		{
			Date check = getDateOfLastSolve(spojadayUsers.get(i));
			if(check.compareTo(bestDate)>0){
				ret = spojadayUsers.get(i) + " is the best spojer. " + getLastSpojSolve(spojadayUsers.get(i));
			}
		}
		return ret;
	}
	...
	public String getWorstSpojer()
	{
		...
		Date worstDate = getDateOfLastSolve("naonao1234");
		for(int i = 0 ; i < spojadayUsers.size();i++)
		{
			Date check = getDateOfLastSolve(spojadayUsers.get(i));
			if(check.compareTo(worstDate)<0){
				ret = spojadayUsers.get(i) + " is a huge slacker. " + getLastSpojSolve(spojadayUsers.get(i));
			}
		}
		return ret;
	}
