...
sub validate_link
{
		...
		for(pattern::get_bad_patterns())
		{
			return 0 if $curr_link =~ /$_/;
		}
		
		for(pattern::get_good_patterns())
		{
			return 1 if $curr_link =~ /$_/;
		}
